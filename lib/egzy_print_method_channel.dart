import 'package:egzy_print/constant.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/services.dart';

import 'egzy_print_platform_interface.dart';

/// An implementation of [EgzyPrintPlatform] that uses method channels.
class MethodChannelEgzyPrint extends EgzyPrintPlatform {
  /// The method channel used to interact with the native platform.
  @visibleForTesting
  final methodChannel = const MethodChannel('egzy_print');
  ...
  Future<String?> getPlatformVersion() async {
    final version = await methodChannel
        .invokeMethod<String>(Constants.deviceInfoGetPlatformVersion);
    return version;
  }
  ...
  Future<bool?> getInstallServiceApk() async {
    final status = await methodChannel
        .invokeMethod<bool>(Constants.deviceInfoGetInstallServiceApk);
    return status;
  }
  ...
  Future<String?> getProperties(String key, String defaultValue) async {
    final value = await methodChannel.invokeMethod<String>(
        Constants.deviceInfoGetProperties, [key, defaultValue]);
    return value;
  }
  ...
  void setDisplayControlStatusBarByBroadcast(bool enable) {
    methodChannel.invokeMethod<void>(
        Constants.displayControlSetStatusBarByBroadcast, [enable]);
  }

  @override
  void setDisplayControlNavigationBarByBroadcast(bool enable) {
    methodChannel.invokeMethod<void>(
        Constants.displayControlSetNavigationBarByBroadcast, [enable]);
  }

  @override
  Future<String?> setDisplayControlStatusBar(bool enable) async {
    final result = await methodChannel
        .invokeMethod<String>(Constants.displayControlSetStatusBar, [enable]);
    return result;
  }
  ...
  Future<String?> setDisplayControlNavigationBar(bool enable) async {
    final result = await methodChannel.invokeMethod<String>(
        Constants.displayControlSetNavigationBar, [enable]);
    return result;
  }

  @override
  Future<String?> getMacAddress() async {
    final macAddress = await methodChannel
        .invokeMethod<String>(Constants.deviceInfoGetMacAddress);
    return macAddress;
  }

  @override
  Future<String?> getSerialNumber() async {
    final serialNumber = await methodChannel
        .invokeMethod<String>(Constants.deviceInfoGetSerialNo);
    return serialNumber;
  }
  ...
  Future<String?> getInternalModel() async {
    final internalModel = await methodChannel
        .invokeMethod<String>(Constants.deviceInfoGetInternalModel);
  ...
  void printLogo(Uint8List image, String type, String name, bool buffer) {
    methodChannel.invokeMethod<void>(Constants.printerPrintLogo,
        {'image': image, "type": type, "name": name, "buffer": buffer});
  }
  ...
  void printBarCode(
    ...
    methodChannel.invokeMethod<void>(Constants.printerPrintBarCode,
        [barcodeStr, bmpWidth, bmpHeight, buffer]);
  }
  ...
  void setThripleHeight(bool thripleHeight) {
    // _printerChannel.invokeMethod(Constants.printerPrintString);
    methodChannel
        .invokeMethod<void>(Constants.printerSetThripleHeight, [thripleHeight]);
  }
